...
public class Level3ScreenView extends SurfaceView implements SurfaceHolder.Callback {
   ...
   * {@link Level3ScreenView}, so you should not draw into the Surface here if your normal rendering
  ...
  public void update() {
    ...
    if ((Manager.getKilledDementorsCount() == 5)
        || roomManager.getSpecialItems().isEmpty()
        || lives == 0) {
      try {
        thread.setRunning(false);
