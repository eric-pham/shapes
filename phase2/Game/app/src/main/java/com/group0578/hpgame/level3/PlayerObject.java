...
class PlayerObject extends Items {
  ...
  void draw(Canvas canvas) {
    if (isCircle) {
      int radius = 25;
      canvas.drawCircle(
          getX() * Level3ScreenView.getCharWidth(),
          getY() * Level3ScreenView.getCharHeight(),
          radius,
          paint);
    } else {
  ...
  /**
   * Causes this item to move horizontally on the screen and to turn around if it reached the edge
   * of the screen.
   *
